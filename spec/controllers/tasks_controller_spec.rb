...
describe TasksController do
  ...
  describe "POST create" do
    describe "with valid params" do
      ...
      it "stores nil if task[completion] is not set" do
        time = mock("Time")
        Time.stub(:now).and_return(time)
        my_task = mock_model(Task).as_null_object
        Task.should_receive(:new).and_return(my_task)
        my_task.should_receive(:completed=).with(nil)
        post :create, {:task => {"completed" => "0"}}, valid_session
      end

      it "stores the current time if task[completion] is set" do
        time = mock("Time")
        Time.stub(:now).and_return(time)
        my_task = mock_model(Task).as_null_object
        Task.should_receive(:new).and_return(my_task)
        my_task.should_receive(:completed=).with(time)
        post :create, {:task => {"completed" => "1"}}, valid_session
      end
